...
public class LikeablePersonService {
    ...
    public RsData<LikeablePerson> like(Member actor, String username, int attractiveTypeCode) {
        ...
        publisher.publishEvent(new EventNewNotify(likeablePerson));
    ...
    public RsData canCancel(Member actor, LikeablePerson likeablePerson) {
        ...
        if (!likeablePerson.isModifyUnlocked()) {
            return RsData.of("F-3", "%s에 취소할 수 있습니다.".formatted(likeablePerson.getModifyUnlockDateRemainStrHuman()));
        }
...
//    @Transactional
    ...
    private void modifyAttractionTypeCode(LikeablePerson likeablePerson, int attractiveTypeCode) {
        ...
        if (rsData.isSuccess()) {
            ...
            publisher.publishEvent(new EventModNotify(likeablePerson, oldAttractiveTypeCode));
        }
    }
    ...
    public RsData canModifyLike(Member actor, LikeablePerson likeablePerson) {
        ...
        if (!likeablePerson.isModifyUnlocked()) {
            return RsData.of("F-3", "%s에 수정할 수 있습니다.".formatted(likeablePerson.getModifyUnlockDateRemainStrHuman()));
        }
